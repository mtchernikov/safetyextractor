...
base = Path("/mnt/data/dspy_prompt_compare_demo")
...
def run_raw_prompt(
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a precise safety engineering extraction assistant. Return valid JSON only."
            },
            {
                "role": "user",
    ...
    return {
        ...
        "raw_output": content,
        "parsed_output": try_parse_json(content),
    }
...
def get_dspy_module(api_key: str, model: str, use_optimizer: bool):
    """
    Create either a plain DSPy predictor or a small compiled predictor.
    The optimizer version costs extra LLM calls on first run.
    """
    import dspy
    ...
    class ExtractSafetyFacts(dspy.Signature):
        ...
        Output valid JSON strings in the output fields.
        """
        ...
        concepts_json = dspy.OutputField(
            desc='JSON array of canonical concept strings, e.g. ["oxygen", "plastic", "gas_path"].'
        )
        relations_json = dspy.OutputField(
            desc='JSON array of [subject, relation, object] triples using only allowed relation IDs.'
        )
        unknowns_json = dspy.OutputField(
            desc='JSON array of missing information questions or unknowns.'
        )
        candidate_inferences_json = dspy.OutputField(
            desc='JSON array of objects with fields fact, basis, needs_validation, confidence.'
        )
    ...
    # Small reviewed examples for demonstration.
    # In a real project, replace these with expert-reviewed cases.
    examples = [
        dspy.Example(
    ...
    # Import path differs slightly across versions, so try both.
    try:
...
st.set_page_config(
    page_title="Prompt vs DSPy Extraction Demo",
    layout="wide",
)
...
if not api_key:
    st.error(
        "No OPENAI_API_KEY found. Add it to Streamlit secrets or your environment."
    )
    st.stop()
...
with st.sidebar:
    ...
    use_optimizer = st.checkbox(
        "Use DSPy BootstrapFewShot optimizer",
        value=False,
        help="This costs extra LLM calls on first run. Leave off for a quick comparison."
    )
...
with st.expander("Normal prompt template"):
    prompt_template = st.text_area(
        ...
        height=360,
        help="Available placeholders: {text}, {allowed_relations}, {ontology_hint}",
    )
